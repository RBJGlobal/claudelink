...
const MCP_SERVER_CONFIG = {
  type: "stdio" as const,
  command: "npx",
  args: ["-y", "-p", "agent-nexus", "agent-nexus-server"],
};
...
function initGlobal() {
  ...
  try {
    execSync("claude mcp add --scope user agent-nexus -- npx -y -p agent-nexus agent-nexus-server", {
      stdio: "inherit",
    });
    console.log(`
  ...
  Run this command manually to add AgentNexus globally:
    ...
    claude mcp add --scope user agent-nexus -- npx -y -p agent-nexus agent-nexus-server
